...
def generate_token(base_url: str, user: str, password: str) -> str:
    ...
    arcpy.AddMessage(f'token_url: {token_url}')
...
def get_token(base_url: str) -> Optional[str]:
    ...
    try:
        ...
        hosted_feature_service: bool = False
        ags_service: bool = True
        portal_url: str = arcpy.GetParameterAsText(0)
        username: str = arcpy.GetParameterAsText(5)
        password: str = arcpy.GetParameterAsText(6)
        
        arcpy.AddMessage(f'portal_url: {portal_url}')
        arcpy.AddMessage(f'username: {username}')
        arcpy.AddMessage(f'password: {password}')
...
def download_service():
    """download service"""
    try:
        srSalida = arcpy.SpatialReference(32719) # wgs84 19S
        zipFileName = datetime.datetime.today().strftime('%Y%m%d%H%M%S')
        jobsDir = arcpy.env.scratchFolder
        jobsGdb = arcpy.env.scratchGDB
        gdbName = os.path.basename(jobsGdb)

        # Creo el archivo .zip
        zip = zipfile.ZipFile(jobsDir + "\\" + zipFileName + ".zip", 'w', zipfile.ZIP_DEFLATED)

        urls: str = arcpy.GetParameterAsText(1)
        arrUrls: Dict[str, Any] = urls.split(";")

        for base_url in arrUrls:
            arcpy.AddMessage(f'base_url: {base_url}')
            where_clause: str = arcpy.GetParameterAsText(2)
            spatial_filter: Any = arcpy.GetParameter(3)
            spatial_relationship: str = arcpy.GetParameterAsText(4)
            # folder_attachments: str = arcpy.GetParameterAsText(7)
            # output_fc: str = arcpy.GetParameterAsText(8)
            arcpy.AddMessage(f'spatial_filter: {spatial_filter}')
            arcpy.AddMessage(f'spatial_relationship: {spatial_relationship}')
            token: Optional[str] = get_token(base_url)
            arcpy.AddMessage(f'token: {token}')
            ...
            arcpy.AddMessage(f'oids: {oids}')


            if oids:
                # Download data
                code = base_url.split('/')[-1]
                serviceName = base_url.split('/')[-3]
                outName = serviceName + '_' + code
                arcpy.AddMessage(f'outName: {outName}')
                fcSalida = arcpy.management.CreateFeatureclass(jobsGdb, outName, "POINT", None,"DISABLED", "ENABLED", srSalida)
                featuresets = download_data(base_url, token, oids, is_layer_table, chunk)
                arcpy.Merge_management(featuresets, fcSalida)
        ...
        for file in os.listdir(jobsGdb):
            arcpy.AddMessage(f'file jobsGdb: {file}')
            zip.write(os.path.join(jobsGdb, file),gdbName+'\\'+os.path.basename(file))
        
        zip.close()

        arcpy.AddMessage("Zip file creado")
        outputFile = jobsDir + "\\" + zipFileName + ".zip"
        arcpy.AddMessage(f'outputFile: {outputFile}')
